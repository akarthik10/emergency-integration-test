...
// FYI: This code was intended as example of how to use the iOS client and not as the API 
// to use in the app. But somehow it became the API that is used.
...
//        self.client = BasicTCPClient(serverHost: host, serverPort: port, isNat: true, natPort: natPort)?
    ...
    @objc public func lookupOrCreateAccount(alias: String, password: String, returnBlock:(resultString : String, error : String?) -> Void) {
        ...
        if (guidEntry == nil) {
            ...
            // Removed this for demo purposes
            /*
            // This removes the default ALL_FIELDS / ALL_GUIDS READ_WHITELIST entry.
            // If this isn't done everyone has access to all fields of our created guid.
            ...
            // make data public again for demo purposes
    ...
    @objc public func verifyAccount(code: String, completion:(result: Bool, error: String) -> Void) {
        if (guidEntry != nil) {
            let (_, verificationError) = client.accountGuidVerify(guidEntry, code: code)
    ...
    @objc public func writeField(field:String, value: AnyObject, completion:(success: Bool, error: String?) -> Void) {
        if (guidEntry != nil) {
            let (_, error) = client.fieldUpdate(guidEntry.guid, field: field, value: value, writer: guidEntry)
            if (error.isEmpty) {
    ...
    @objc public func updateFieldsFromJSON(value: String, completion: (result: Bool, error: String) -> Void) {
        ...
        if (guidEntry != nil) {
            ...
            let (_, error) = client.update(guidEntry.guid, json: json, writer: guidEntry)
            if (error.isEmpty) {
                return completion(result: true, error: "")
            } else {
...
//            print("accesserGuid: %@", accesserGuid);
            let aclReturn = self.client.aclAdd(aType, targetGuid: guidEntry, field: GNSProtocol.ENTIRE_RECORD, accesserGuid: accesserGuid)
